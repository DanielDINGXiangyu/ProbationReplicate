function [] = plot_dePolicy_noNormal(a,b,c,RRA,IES,name)
...
%   Detailed explanation goes here
    ...
    for ii=1:length(IES)
        ...
        plot(B, bp(:, aa), 'r','Linewidth',1.5)
        hold on
        plot(B, bp(:, bb), 'g','Linewidth',1.5)
        hold on
        plot(B, bp(:, cc), 'b','Linewidth',1.5)
        hold off
        ylim([-1.1,-0.4]) 
        xlim([-1.1,-0.4]) 
        legend({sprintf('Low %s: DE', str), sprintf('Mid %s: DE', str), sprintf('High %s: DE', str)},'Location','southeast')
        xlabel('Current Detrended Bond: $\frac{b_t}{\Gamma_{t-1}}$','interpreter', 'latex')
