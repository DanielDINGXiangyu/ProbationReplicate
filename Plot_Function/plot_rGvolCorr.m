...
function [] = plot_rGvolCorr(a,b,RRA,IES,name)
% Plot figure: world interest rate and growth volaitlity correlation
    ...
    for ii=1:length(IES)
        ...
        load(sprintf('Solution/LowBound/rgcorr_rra%02dies%02d_spreadR_lowsticky.mat', RRA(ii), round(IES(ii)*10)));
    ...
    saveas(gcf,append(append('Plot/3R/',name),'_withCorr.png'))
    ...
    saveas(gcf,append(append('Plot/3R/',name),'_NoCorr.png'))
end
