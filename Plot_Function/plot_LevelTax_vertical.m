function [] = plot_LevelTax_vertical(a,b,c,d, RRA,IES,name)
% Plot figure: volatility tax across z and g volatility regime
    ...
    for ii=1:length(IES)
        ...
        ylim([0,20]) 
        xlim([-1.1,-0.4]) 
        legend({'Low G Level: SP', 'High G Level: SP'},'Location','southeast')
        xlabel('Bond/Current Trend: $\frac{b_t}{\Gamma_{t}}$','interpreter', 'latex')
        ylabel("Macroprudential Tax $\tau$ (\%) ",'interpreter', 'latex')
        title(sprintf('Across G Level; γ=%d ψ=%.1f ', RRA(ii), IES(ii)))
